...
const accommodationDetails={"21 July": {"name": "Bergen accommodation", "type": "Accommodation check-in", "description": "Settle into the Bergen stay, leave the luggage safely and prepare a light day bag before beginning the city programme.", "transport": "Use Bergen Light Rail or the confirmed airport transfer. Check the final walking route in Google Maps after arrival.", "image": "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1400&q=84", "map": "Bergen+city+centre"}, "23 July": {"name": "Scandic Byporten, Oslo", "type": "Hotel check-in", "description": "The Oslo hotel is connected to Oslo Central Station, making it convenient for arriving by train and starting the waterfront itinerary.", "transport": "Follow station signs toward Byporten shopping centre and the hotel entrance. The walk is indoors or mostly covered.", "image": "https://images.unsplash.com/photo-1564501049412-61c2a3083791?auto=format&fit=crop&w=1400&q=84", "map": "Scandic+Byporten+Oslo"}, "25 July": {"name": "Copenhagen accommodation", "type": "Accommodation check-in", "description": "Drop the luggage, refresh and carry only essentials before starting the Nyhavn and canal-cruise programme.", "transport": "Use Rejseplanen for the fastest route from the cruise terminal. Confirm the exact stop and walking route privately.", "image": "https://images.unsplash.com/photo-1611892440504-42a792e24d32?auto=format&fit=crop&w=1400&q=84", "map": "Copenhagen+city+centre"}};
      ...
      <div class="day-collapsible">
        <div class="schedule premium-timeline">
          ${day.schedule.map((item,itemIndex)=>{
            ...
            const isCheckIn=/check[\s-]?in|airport transfer and check-in|arrive and check in/i.test(item[1]);
            const stayDetail=isCheckIn ? accommodation : null;
            if(detail) sightCounter += 1;

            return `
            <div class="schedule-row ${stayDetail?'accommodation-entry':'sightseeing-entry'}">
              <div class="schedule-time"><span>${item[0]}</span><i></i></div>
              <div class="schedule-copy">
                ${stayDetail ? `
                  <div class="image-wrap accommodation-image-wrap">
                    <img class="sight-image accommodation-image" src="${stayDetail.image}" alt="${stayDetail.name} accommodation preview" loading="lazy">
                    <span class="image-badge accommodation-badge">Accommodation</span>
                  </div>
                  <div class="sight-heading">
                    <h4>${stayDetail.name}</h4>
                    <span class="stay-badge">${stayDetail.type}</span>
                  </div>
                  <p class="sight-description">${stayDetail.description}</p>
                  <div class="detail-box plan-box"><strong>Check-in plan</strong><p>${item[2]}</p></div>
                  <div class="detail-box route-box"><strong>Getting to the accommodation</strong><p>${stayDetail.transport}</p></div>
                  <a class="map-button stay-map" target="_blank" rel="noopener" href="https://www.google.com/maps/search/?api=1&query=${stayDetail.map}">Open accommodation area map <span>↗</span></a>
                  <p class="illustrative-note">Public-safe illustrative accommodation image; exact property photos and address are intentionally not published.</p>
                ` : `
                  ${detail?`<div class="image-wrap"><img class="sight-image" src="${detail.image}" alt="${item[1]}" loading="lazy"><span class="image-badge">Sight ${sightCounter}</span></div>`:""}
                  <div class="sight-heading">
                    <h4>${item[1]}</h4>
